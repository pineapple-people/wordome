...
class SitemapPdpDiscovererService:
    """
    ...
    async def discover_pdp_urls(
        ...
        *,
        ...
        effective_max_depth = (
            ...
            else (profile.max_depth if profile else 3)
        )
        ...
        while queue and len(processed_sitemaps) < effective_max_sitemaps:
            ...
            xml_text = await self._web_fetcher.fetch(normalized_sitemap_url)
            if not xml_text:
                errors.append(f"failed to fetch sitemap: {normalized_sitemap_url}")
                self._observe(
                    record_observer,
                    SitemapCrawlRecordObservation(
                        record_url=normalized_sitemap_url,
            ...
            if parsed_sitemap is None:
                errors.append(f"failed to parse sitemap: {normalized_sitemap_url}")
                self._observe(
                    record_observer,
                    SitemapCrawlRecordObservation(
                        record_url=normalized_sitemap_url,
            ...
            for nested_sitemap_url in parsed_sitemap["sitemaps"]:
                ...
                should_follow, skip_reason = self._should_follow_sitemap(
                    normalized_nested,
                    root_host=root_host,
                    same_host_only=profile.same_host_only if profile else True,
                    include_regexes=sitemap_include_regexes,
                    exclude_regexes=sitemap_exclude_regexes,
                )
                if should_follow:
                    queue.append((normalized_nested, depth + 1, normalized_sitemap_url))
                else:
                ...
                if self._matches_pdp_url(
                    ...
                    self._observe(
                        ...
                        SitemapCrawlRecordObservation(
                            ...
                            parent_url=normalized_sitemap_url,
                            record_type="discovered_url",
                            url_type=self._classify_url_type(normalized_candidate),
                            depth=depth + 1,
                            record_status="skipped",
